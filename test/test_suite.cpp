...
void test_buffer_size_hint() {
	...
	for (const auto &c : cases) {
		size_t size = get_ep_buffer_size_hint(c.max_tokens, c.hidden,
											  c.num_ranks, c.num_experts);
...
void test_ibgda_disabled_fallback() {
	...
	std::cout << "IBGDA disabled by env: " << (disabled ? "yes" : "no")
			  << std::endl;
	std::cout << "PASSED!" << std::endl;
}
...
void test_repeated_operations(EpBuffer &ep, int num_ranks, int rank) {
	...
	std::cout << "Running 3 iterations of update_local_qpns + get_local_qpns..."
			  << std::endl;
