...
template<typename T>
bool send_vector(int sock, const std::vector<T>& vec) {
    uint32_t size = vec.size();
    if (!send_all(sock, &size, sizeof(size))) return false;
    if (size > 0 && !send_all(sock, vec.data(), size * sizeof(T))) return false;
    return true;
}

template<typename T>
bool recv_vector(int sock, std::vector<T>& vec) {
    uint32_t size;
    if (!recv_all(sock, &size, sizeof(size))) return false;
    vec.resize(size);
    if (size > 0 && !recv_all(sock, vec.data(), size * sizeof(T))) return false;
    return true;
}
...
bool exchange_rank_info(int my_rank, int num_ranks, const std::vector<std::string>& peer_ips, RankInfo& my_info, std::vector<RankInfo>& all_info) {
    ...
    if (my_rank == 0) {
        ...
        for (int i = 1; i < num_ranks; ++i) {
            ...
            std::cout << "[Rank 0] Accepted connection from rank " << i << std::endl;
        }
        ...
        for (int i = 1; i < num_ranks; ++i) {
            int sock = client_socks[i-1];
            if (!send_all(sock, all_info.data(), num_ranks * sizeof(RankInfo))) return false;
        ...
        for (int sock : client_socks) close(sock);
        close(server_sock);
        ...
        if (!send_vector(client_sock, my_info.qpns)) return false;
        if (!send_vector(client_sock, my_info.lids)) return false;
        if (!send_all(client_sock, &my_info.mr_addr, sizeof(int64_t))) return false;
        if (!send_all(client_sock, &my_info.mr_rkey, sizeof(int32_t))) return false;
        if (!send_all(client_sock, &my_info.gid_subnet_prefix, sizeof(int64_t))) return false;
        if (!send_all(client_sock, &my_info.gid_interface_id, sizeof(int64_t))) return false;
        if (!send_vector(client_sock, my_info.ipc_handle)) return false;
        if (!send_all(client_sock, &my_info.is_roce, sizeof(bool))) return false;
        
        all_info.resize(num_ranks);
        
        for (int j = 0; j < num_ranks; ++j) {
            if (!recv_vector(client_sock, all_info[j].qpns)) return false;
            if (!recv_vector(client_sock, all_info[j].lids)) return false;
            if (!recv_all(client_sock, &all_info[j].mr_addr, sizeof(int64_t))) return false;
            if (!recv_all(client_sock, &all_info[j].mr_rkey, sizeof(int32_t))) return false;
            if (!recv_all(client_sock, &all_info[j].gid_subnet_prefix, sizeof(int64_t))) return false;
            if (!recv_all(client_sock, &all_info[j].gid_interface_id, sizeof(int64_t))) return false;
            if (!recv_vector(client_sock, all_info[j].ipc_handle)) return false;
            if (!recv_all(client_sock, &all_info[j].is_roce, sizeof(bool))) return false;
        }
...
void test_basic_init(EpBuffer& ep, int rank) {
    print_separator("Test 1: Basic Initialization [Rank " + std::to_string(rank) + "]");
...
void test_sync_ib(EpBuffer& ep, int rank, const std::vector<RankInfo>& all_info, const std::vector<int>& active_ranks) {
    ...
    for (size_t i = 0; i < all_info.size(); ++i) {
        ...
        remote_qpns.push_back(all_info[i].qpns.empty() ? 0 : all_info[i].qpns[0]);
        remote_lids.push_back(all_info[i].lids.empty() ? 0 : all_info[i].lids[0]);
    }
    
    std::cout << "Calling sync_ib..." << std::endl;
    ep.sync_ib(remote_addrs, remote_keys, remote_qpns, remote_lids, active_ranks);
    std::cout << "sync_ib completed" << std::endl;
    std::cout << "PASSED!" << std::endl;
}

void test_sync_roce(EpBuffer& ep, int rank, const std::vector<RankInfo>& all_info, const std::vector<int>& active_ranks) {
    ...
    ep.sync_roce(remote_addrs, remote_keys, remote_qpns, subnet_prefixes, interface_ids, active_ranks);
    std::cout << "sync_roce completed" << std::endl;
    std::cout << "PASSED!" << std::endl;
}

void test_nvlink_ipc(EpBuffer& ep, int rank, const std::vector<RankInfo>& all_info, const std::vector<int>& active_ranks) {
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (num_ranks > 1) {
            ...
            if (!exchange_rank_info(rank, num_ranks, peer_ips, my_info, all_info)) {
