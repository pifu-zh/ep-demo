...
int main(int argc, char **argv) {
	...
	try {
		...
		std::cout << "[Test] IBGDA disabled: "
				  << (ep_buffer.ibgda_disabled() ? "yes" : "no") << std::endl;
		std::cout << "[Test] Is RoCE: " << (ep_buffer.is_roce() ? "yes" : "no")
				  << std::endl;
		std::cout << "[Test] Use fast path: "
				  << (ep_buffer.use_fast_path() ? "yes" : "no") << std::endl;

		if (!ep_buffer.ibgda_disabled()) {
			auto [addr, rkey] = ep_buffer.get_mr_info();
			std::cout << "[Test] MR addr: 0x" << std::hex << addr << std::dec
					  << ", rkey: 0x" << std::hex << rkey << std::dec
					  << std::endl;

			auto [subnet_prefix, interface_id] = ep_buffer.get_gid();
			std::cout << "[Test] GID: subnet_prefix=0x" << std::hex
					  << subnet_prefix << ", interface_id=0x" << interface_id
					  << std::dec << std::endl;
		...
		std::cout << "[Test] IPC handle size: " << ipc_handle.size()
				  << std::endl;
